...
parseROM :: ROM -> Cartridge
parseROM v = Cartridge { .. }
  where
    (!) = (V.!)
    read = V.fromList . map (v !)
    entryPoint = read [0x100..0x103]
    logo = read [0x104..0x133]
    title = map (chr . fromIntegral . (v !)) [0x134..0x143]
    manufacturerCode = read [0x13f..0x142]
    cgbFlag = cgbFlagTable $ v ! 0x143
    newLicenseeCode = newLicenseeTable $ map (chr . fromIntegral . (v !)) [0x144..0x145]
    sgbFlag = sgbFlagTable $ v ! 0x146
    mbcType = mbcTypeTable $ v ! 0x147
    romSize = romSizeTable $ v ! 0x148
    ramxSize = ramxSizeTable $ v ! 0x149
    destinationCode = destinationCodeTable $ v ! 0x14a
    oldLicenseeCode = oldLicenseeCodeTable $ v ! 0x14b
    maskROMVersionNumber = v ! 0x14c
    headerChecksum = v ! 0x14d
    globalChecksum = shift (fromIntegral $ v ! 0x14e) 8 .|.  (fromIntegral $ v ! 0x14f)
    raw = v
