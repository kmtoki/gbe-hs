...
readOP8 op = do
  case op of
    ...
    L -> use $ cpu.l
    W -> readPC
    P_BC -> (use $ cpu.bc) >>= read
    P_DE -> (use $ cpu.de) >>= read
    P_HL -> (use $ cpu.hl) >>= read
    P_WW -> (flip toWW <$> readPC <*> readPC) >>= read . toInt
    P_FF00_C -> (use $ cpu.c) >>= read . (0xff00 +) . toInt
...
writeOP8 op w = do
  case op of
    ...
    H -> cpu.h .= w
    L -> cpu.l .= w
    P_BC -> (use $ cpu.bc) >>= flip write w
    P_DE -> (use $ cpu.de) >>= flip write w
    P_HL -> (use $ cpu.hl) >>= flip write w
    P_WW -> (flip toWW <$> readPC <*> readPC) >>= flip write w
    P_FF00_C -> (use $ cpu.c) >>= flip write w . (0xff00 +) . toInt
...
condFlag op = do
  cpu' <- use cpu
  pure $ case op of
    NotZero -> not $ cpu'^.zero
    Zero -> cpu'^.zero
    NotCarry -> not $ cpu'^.carry
    Carry -> cpu'^.carry
    Always -> True
    _ -> error $ "condFlag: unexpect OP " ++ show op
...
dispatch :: GB ()
dispatch = do
  (ITable it) <- use $ cpu.iTable
  instruction <- readPC
  if instruction == 0x10 || instruction == 0xcb then do
    instruction' <- readPC
    it V.! ((fi instruction `shiftL` 8) .|. fi instruction')
  else do
    it V.! fi instruction
      

newITable :: ITable
newITable = ITable (V.replicate 0xcc00 nop V.// ts)
  where
    ts = [
      (0x3e, ld8 A W),
      (0x06, ld8 B W),
